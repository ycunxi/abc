...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap7.]

  Description [Iteration 7 keeps stmap5's selective skip-fanout
  thresholds but adds a small default area bias. The hypothesis is that
  a mild `AreaMulti` penalty can recover some large-design area without
  reopening the timing regressions seen in weaker threshold variants.]
...
***********************************************************************/
int Abc_CommandStmap7( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap7.\n" );
...
usage:
    ...
    Abc_Print( -2, "usage: stmap7 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 7 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from stmap5 by enabling a small area multiplier of 0.10 while keeping the selective thresholds unchanged\n" );
