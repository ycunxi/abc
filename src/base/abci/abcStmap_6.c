...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap6.]

  Description [Iteration 6 tightens stmap5's relaxed 3+-leaf threshold
  from 5 refs to 4 refs while keeping the stricter 4+-leaf threshold.
  The hypothesis is that large-design area can recover without giving
  back too much of stmap5's delay improvement.]
...
***********************************************************************/
int Abc_CommandStmap6( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap6.\n" );
        pNtkRes = Abc_NtkMapSelectiveFanout( pNtk, NULL, DelayTarget, AreaMulti, DelayMulti, LogFan, Slew, Gain, nGatesMin, fRecovery, fSwitching, fSkipFanout, 4, 1, fUseProfile, fUseBuffs, fVerbose );
        if ( pNtkRes == NULL )
        {
            Abc_NtkDelete( pNtk );
            Abc_Print( -1, "stmap6 has failed.\n" );
            return 1;
        }
        Abc_NtkDelete( pNtk );
    }
...
usage:
    ...
    Abc_Print( -2, "usage: stmap6 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 6 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from stmap5 by tightening the relaxed 3-leaf skip-fanout threshold from 5 refs to 4 refs\n" );
