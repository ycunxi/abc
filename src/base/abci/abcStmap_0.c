...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap0.]

  Description [Iteration 0 bootstraps the stmap family as a conservative
  wrapper around the classic mapper. Its hypothesis is that preserving
  output buffering opportunities and avoiding oversized matches on
  high-fanout nodes can improve the downstream topo/buffer/upsize/dnsize
  flow without changing mapper internals yet.]
...
***********************************************************************/
int Abc_CommandStmap0( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap0.\n" );
...
usage:
    ...
    Abc_Print( -2, "usage: stmap0 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 0 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from map by enabling -f and -o\n" );
