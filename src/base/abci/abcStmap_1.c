...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap1.]

  Description [Iteration 1 keeps the conservative stmap0 wrapper defaults
  and additionally enables a logarithmic fanout-delay estimate. The
  hypothesis is that making fanout pressure visible during matching will
  better align mapping decisions with the downstream topo/buffer/upsize/
  dnsize flow on larger designs.]
...
***********************************************************************/
int Abc_CommandStmap1( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap1.\n" );
...
usage:
    ...
    Abc_Print( -2, "usage: stmap1 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 1 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from map by enabling -F 1.0, -f, and -o\n" );
