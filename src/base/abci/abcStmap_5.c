...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap5.]

  Description [Iteration 5 keeps stmap4's relaxed threshold for 3+-leaf
  cuts but restores the stricter 4+-leaf threshold used by stmap1. The
  hypothesis is that moderate fanout flexibility helps i10, while
  keeping 4-leaf cuts on a tighter leash avoids stmap4's or1200 delay
  regression.]
...
***********************************************************************/
int Abc_CommandStmap5( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap5.\n" );
        pNtkRes = Abc_NtkMapSelectiveFanout( pNtk, NULL, DelayTarget, AreaMulti, DelayMulti, LogFan, Slew, Gain, nGatesMin, fRecovery, fSwitching, fSkipFanout, 5, 1, fUseProfile, fUseBuffs, fVerbose );
        if ( pNtkRes == NULL )
        {
            Abc_NtkDelete( pNtk );
            Abc_Print( -1, "stmap5 has failed.\n" );
            return 1;
        }
        Abc_NtkDelete( pNtk );
    }
...
usage:
    ...
    Abc_Print( -2, "usage: stmap5 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 5 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from stmap4 by restoring the stricter 4-leaf skip-fanout threshold while keeping the relaxed 3-leaf threshold\n" );
