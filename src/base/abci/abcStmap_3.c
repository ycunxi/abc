...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap3.]

  Description [Iteration 3 keeps stmap1's logarithmic fanout-delay
  estimate enabled at 1.0, but disables the default skip-fanout gate
  filter. The hypothesis is that removing the hard high-fanout gate
  skip while keeping the softer delay estimate will reduce the compound
  fanout pressure that hurt i10 in stmap1.]
...
***********************************************************************/
int Abc_CommandStmap3( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap3.\n" );
...
usage:
    ...
    Abc_Print( -2, "usage: stmap3 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 3 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from map by enabling -F 1.0 and -o while leaving -f off\n" );
