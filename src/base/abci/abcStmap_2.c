...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap2.]

  Description [Iteration 2 keeps the conservative stmap0 wrapper defaults
  and keeps the logarithmic fanout-delay estimate enabled, but reduces
  the default value from 1.0 to 0.5. The hypothesis is that softer
  fanout pressure will preserve most of stmap1's large-design gains
  while recovering the small-design regression observed on i10.]
...
***********************************************************************/
int Abc_CommandStmap2( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap2.\n" );
...
usage:
    ...
    Abc_Print( -2, "usage: stmap2 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 2 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from map by enabling -F 0.5, -f, and -o\n" );
