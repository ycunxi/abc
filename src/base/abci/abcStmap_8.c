...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap8.]

  Description [Iteration 8 keeps stmap5's selective skip-fanout
  thresholds and uses a smaller default area bias than stmap7. The
  hypothesis is that `AreaMulti=0.05` can recover some area without the
  heavier cross-benchmark penalties seen at `0.10`.]
...
***********************************************************************/
int Abc_CommandStmap8( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap8.\n" );
...
usage:
    ...
    Abc_Print( -2, "usage: stmap8 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 8 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from stmap7 by reducing the area multiplier to 0.05 while keeping the selective thresholds unchanged\n" );
