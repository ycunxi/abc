...
/**Function*************************************************************
  ...
  Synopsis    [Runs stmap9.]

  Description [Iteration 9 keeps stmap5's selective skip-fanout
  thresholds and uses a very small default area bias. The hypothesis is
  that `AreaMulti=0.02` may preserve some useful area recovery while
  avoiding most of the penalties still present in stmap8.]
...
***********************************************************************/
int Abc_CommandStmap9( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( !Abc_NtkIsStrash( pNtk ) )
    {
        ...
        Abc_Print( 0, "The network was strashed and balanced before stmap9.\n" );
...
usage:
    ...
    Abc_Print( -2, "usage: stmap9 [-DABFSG float] [-M num] [-arspfuovh]\n" );
    Abc_Print( -2, "\t              performs version 9 downstream-aware standard cell mapping\n" );
    Abc_Print( -2, "\t              defaults differ from stmap8 by reducing the area multiplier to 0.02 while keeping the selective thresholds unchanged\n" );
